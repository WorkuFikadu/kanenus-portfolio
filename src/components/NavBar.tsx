...
export default function NavBar() {
  ...
  // If on the administrative dashboard, hide the public website navbar
  if (pathname.startsWith('/dashboard')) {
    return null;
  }

  return (
    <header className="bg-[#0b1a30]/95 dark:bg-gray-950/95 backdrop-blur-md text-white py-4 sticky top-0 z-50 border-b border-white/10 shadow-lg">
              ...
              <>
                <Link
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                    pathname.startsWith('/dashboard') ? 'text-accent bg-white/10 font-bold' : 'text-gray-300 hover:text-accent hover:bg-white/10'
                  }`}
                >
      ...
      <div className={`lg:hidden transition-all duration-300 overflow-hidden ${menuOpen ? 'max-h-screen' : 'max-h-0'}`}>
        <div className="bg-[#071324] border-t border-white/10 px-6 py-4 flex flex-col gap-1">
          ...
          {loggedIn ? (
            <>
              <Link href="/dashboard" onClick={() => setMenuOpen(false)} className="px-4 py-3 rounded-xl text-sm font-medium text-accent bg-white/5">Dashboard</Link>
              <button onClick={handleLogout} className="px-4 py-3 rounded-xl text-sm font-medium text-red-400 text-left">Logout</button>
            </>
          ) : (
            <Link href="/login" onClick={() => setMenuOpen(false)} className="px-4 py-3 rounded-xl text-sm font-medium text-gray-300 hover:text-accent hover:bg-white/10">Login</Link>
          )}
